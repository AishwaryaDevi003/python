...
# Write a class MacroProcessor that interprets custom macros in text and replaces them using rules stored in a dictionary.
# Each macro starts with {{ and ends with }}.
# Some macros can call others recursively (nested macro expansion).
...
#   "greet": "Hello {{name}}!",
#   "name": "Ravi",
#   "intro": "{{greet}} Welcome to {{place}}.",
#   "place": "Python Academy"
# }
# processor = MacroProcessor(macros)
# processor.expand("{{intro}}")

# Expected Output:

# "Hello Ravi! Welcome to Python Academy."

# (Bonus: Detect infinite recursion like A → B → A and raise an exception.)
...
class MacroProcessor:
    ...
    def expand(self, text, seen=None):
        if seen is None:
            seen = set()
        pattern = r'\{\{(.*?)\}\}'
        ...
        def replace_macro(match):
            ...
            if macro_name in seen:
                raise ValueError(f'Infinite recursion detected for macro: {macro_name}')
            if macro_name in self.macros:
                seen.add(macro_name)
                expanded = self.expand(self.macros[macro_name], seen)
                seen.remove(macro_name)
                return expanded
            return match.group(0)

        while re.search(pattern, text):
            text = re.sub(pattern, replace_macro, text)
        return text
macros = {
  "greet": "Hello {{name}}!",
    "name": "Ravi",
